...
# Fixed version of build-docker.sh using a specific Docker image tag
# Ensures script fails if any command fails
set -euo pipefail
...
# Use a specific Docker image version that's known to work with SDK 2.7.0
DOCKER_IMAGE="ghcr.io/zephyrproject-rtos/ci:v0.26.13"
...
echo -e "${YELLOW}Pulling specific Docker image: $DOCKER_IMAGE${NC}"
...
docker run --rm $PLATFORM_FLAG \
    ...
    $DOCKER_IMAGE \
    bash -c "pip install --user --break-system-packages adafruit-nrfutil 2>/dev/null || pip install --user adafruit-nrfutil && \
